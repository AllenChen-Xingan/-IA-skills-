...
import type { DiffStats, HookInput, HookConfig } from './types.js';
 ...
 * 使用 spawnSync 避免命令注入
 */
export function gitPush(): boolean {
  try {
    // 推送代码
    ...
    if (pushResult.status !== 0) {
      log(`git push failed: ${pushResult.stderr}`, 'error');
      return false;
    }

    log('Pushed commits to remote', 'info');

    // 推送 tags
    const pushTagsResult = spawnSync('git', ['push', 'origin', '--tags'], {
      stdio: 'pipe',
      encoding: 'utf8',
    });

    if (pushTagsResult.status !== 0) {
      log(`git push tags failed: ${pushTagsResult.stderr}`, 'error');
      return false;
    }

    log('Pushed tags to remote', 'info');
    return true;
  } catch (e) {
    log(`git push error: ${e}`, 'error');
    return false;
  }
}
